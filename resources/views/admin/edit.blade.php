...
<div class="text-center bg-danger text-white p-5">
    ...
    <form action="{{ url('admin/'.$reservation->id) }}" method="POST">
       @csrf
       @method('PUT')
        ...
        <div class="row m-4 ">
            ...
            value="{{$reservation->first_name ?? old('first_name')}}"   required>
            </div>
            <div id="hov" class="form-group mb-3">
            <label> Address</label>
            <textarea name='address' class="form-control" rows="1"
            value="{{$reservation->address ?? old('address')}}" required></textarea>
            </div>
            <div id="hov" class="form-group mb-3">
            <label> City</label>
            <input type="text" name='city' class="form-control"
            value="{{$reservation->city ?? old('city')}}" required>
            </div>
            <div id="hov" class="form-group mb-3">
            <label> Email</label>
            <input type="email" name='email' class="form-control" 
            value="{{$reservation->email ?? old('email')}}" required>
            </div>
            ...
            value="{{$reservation->last_name ?? old('last_name')}}" required >
            </div>
            <div id="hov" class="form-group mb-3">
            <label> Zip Code</label>
            <input type="number" name='zip_code' class="form-control"
            value="{{$reservation->zip_code ?? old('zip_code')}}"  required>
            </div>
            <div id="hov" class="form-group mb-3">
            <label> State </label>
            <input type="text" name='state' class="form-control"
            value="{{$reservation->state ?? old('state')}}"  required>
            </div>
            <div id="hov" class="form-group mb-3">
            <label> Phone</label>
            <input type="text" name='phone' class="form-control"
            value="{{$reservation->phone ?? old('phone')}}"  required>
            </div>
            </div>
        ...
        <div class="card-header bg-danger">
            ...
            <div  id="hov" class="form-group mb-5">
                    ...
                    <input class="form-control" type="date" name="check_out_date"
        ...
        <div class="row  m-4">
            <div class="col-md-12">
                <div class="form-group">
                    ...
                    value="{{$reservation->room ?? old('room')}}">
                </div>
            </div>
        </div>
    ...
    <button  class="btn btn-danger btn-lg">Update</button>
    </div>
    </form>
